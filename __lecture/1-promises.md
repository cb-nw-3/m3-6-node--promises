...
- It creates a separate thread for anything I/O (input/output).
  - read/write to disk
...
callMyself();
// recurcive function s'appelle elle même
```
...
2. Parse(traiter) the HTTP request to understand it
...
<!-- asynchronous : plusieurs requête
synchronous : arrêt après une requête -->
